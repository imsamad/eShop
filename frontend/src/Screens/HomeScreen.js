...
const HomeScreen = ({match}) => {
  ...
  return (
    ...
    <Meta title='Welcome to eShop | Home' />
      ...
      {keyword && <Link to="/" className="btn btn-success btn-sm" >Go Back</Link>}
      <h1 className="text-success">Latest Products</h1>
      <Paginate page={page} pages={pages} keyword={keyword} />
      <Row>
        {loading ? (
          <Col><Loader w='8rem' h='4px'/></Col>):error ? 
          (<Col>
              <Alert variant="danger">{error}</Alert>
          </Col>
        ):(
          products.map(product=>(
            <Col   key={product._id}  className="p-2 p-sm-0  " sm={6} md={4} lg={4} xl={3}>
