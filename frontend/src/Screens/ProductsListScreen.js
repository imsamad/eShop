...
import { productLists,deleteProduct, createProduct} from '../actions/productActions'
...
const ProductsListScreen = ({history,match}) => {    
    ...
    const {error:deleteError,loading:deleteLoading,success,message:deleteMessage}=productDelete
    
    const productCreate=useSelector(state=>state.productCreate)
    const {success:successCreate,error:createError,loading:createLoading,product}=productCreate
    useEffect(()=>{
    if(successCreate){
    ...
    <Container >
      <Row className="align-items-center" >
        <Col><h2 className="text-success">Products</h2></Col>
      ...
      <Row>
        <Col className="mx-auto">
          ...
          {deleteLoading ? (<Progress/>):deleteError ? (
            ...
            <tbody>
              ...
              <td>{product.brand}</td>
                <td>
                  ...
                  <td> 
                  <Button className="btn btn-sm btn-danger" onClick={()=>deleteHandler(product._id)} ><i className="fa fa-trash" ></i></Button>
                </td>
              </tr>
